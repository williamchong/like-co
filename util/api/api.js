...
export const apiGetLikeButtonMyStatus = (id, referrer) => axios.get(`/like/likebutton/${id}/self`, { params: { referrer } });
export const apiGetLikeButtonTotalCount = (id, referrer) => axios.get(`/like/likebutton/${id}/total`, { params: { referrer } });
export const apiGetLikeButtonLikerList = (id, referrer) => axios.get(`/like/likebutton/${id}/list`, { params: { referrer } });
export const apiPostLikeButton = (id, referrer, count = 1) => axios.post(
  `/like/likebutton/${id}/${count}`,
  {},
  { params: { referrer } },
);

export const apiGetLikeStatistic = () => axios.get('/like/like/stat');
export const apiGetLikeArticleInfo = url => axios.get(`/like/like/suggest/info/?url=${encodeURIComponent(url)}`);
export const apiGetLikeURLSuggestion = () => axios.get('/like/like/suggest/all', { withCredentials: false });
export const apiGetLikeURLPersonalSuggestion = () => axios.get('/like/like/suggest/personal');
export const apiGetUserLikeAmount = () => axios.get('/like/info/like/amount');
...
export const apiGetOAuthAuthorize = (clientId, redirectUri, scope, opt) => {
  ...
  return axios.get(`/oauth/authorize?client_id=${clientId}&scope=${scopeString}&redirect_uri=${encodeURIComponent(redirectUri)}`, opt);
};
export const apiPostOAuthAuthorize = payload => axios.post('/oauth/authorize', payload);
